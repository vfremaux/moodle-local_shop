...
class shop_shipzones_renderer extends local_shop_base_renderer {
    ...
    public function shippings($shippings) {
        ...
        foreach ($shippings as $shipping) {
            ...
            $commands = '<a href="'.$cmdurl.'">'.$OUTPUT->pix_icon('t/edit', get_string('edit'), 'core').'</a>';

            $params = array('what' => 'deleteshipping', 'shipid[]' => $shipping->id);
            $cmdurl = new moodle_url('/local/shop/shipzones/zoneindex.php', $params);
            $commands .= '&nbsp;<a href="'.$cmdurl.'">'.$OUTPUT->pix_icon('t/delete', get_string('delete'), 'core').'</a>';
            $row[] = $commands;
    ...
    public function zones($zones) {
        ...
        foreach ($zones as $z) {
            ...
            if ($z->entries == 0) {
                ...
                $commands = '<a href="'.$indexurl.'">'.$OUTPUT->pix_icon('t/delete', get_string('delete'), 'core').'</a>';
            ...
            $commands .= '&nbsp;<a href="'.$editzoneurl.'">'.$OUTPUT->pix_url('t/edit', get_stirng('edit'), 'core').'</a>';
            $row[] = $commands;
    ...
    public function catalogitem_shipping_zones($zones) {
        ...
        foreach ($zones as $z) {
            ...
            if ($z->entries == 0) {
                ...
                $indexurl = new moodle_url('/local/shop/shipzones/index.php', $params);
                $commands = '<a href="'.$indexurl.'">'.$OUTPUT->pix_url('t/delete', get_string('delete')).'</a>';
            ...
            $commands .= ' <a href="'.$zoneurl.'">'.$OUTPUT->pix_url('t/edit', get_string('edit'), 'core').'</a>';
