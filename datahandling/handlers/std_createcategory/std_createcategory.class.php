    ...
    public function produce_prepay(&$data) {
        ...
        if (isloggedin()) {
            ...
            if (!shop_create_customer_user($data, $customer, $newuser)) {
                $message = "[{$data->transactionid}] {$this->name} Prepay Error :";
            ...
            $message = get_string('productiondata_sales', 'shophandlers_std_createcategory', $newuser->username);
            $productionfeedback->salesadmin = $message;
        }
