...
class shop_handler_std_createvinstance extends shop_handler {
    ...
    public function produce_prepay(&$data) {
        ...
        if (isloggedin()) {
            if ($customer->hasaccount != $USER->id) {
                ...
                $message = "[{$data->transactionid}] STD_CREATE_VINSTANCE Prepay :";
            ...
            /*
             * In this case we can have a early Customer that never confirmed a product
             * or a brand new Customer comming in.
            ...
            if (!shop_create_customer_user($data, $customer, $newuser)) {
                $message = "[{$data->transactionid}] STD_CREATE_VINSTANCE Prepay Error :";
            ...
            $fb = get_string('productiondata_private', 'shophandlers_std_createvinstance', $a);
            $productionfeedback->private = $fb;
            $fb = get_string('productiondata_sales', 'shophandlers_std_createvinstance', $newuser->username);
    ...
    public function produce_postpay(&$data) {
        ...
        if (!$preprocesserror) {
            ...
            $fb = get_string('productiondata_failure_public', 'shophandlers_std_createvinstance', 'Code : CATEGORY CREATION');
