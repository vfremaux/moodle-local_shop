...
class shop_handler_std_addtrainingcredits extends shop_handler {
    ...
    public function produce_postpay(&$data) {
        ...
        if (is_dir($CFG->dirroot.'/enrol/trainingcredits')) {
            ...
            $fb = get_string('productiondata_assign_public', 'shophandlers_std_addtrainingcredits', '');
            $productionfeedback->public = $fb;
            $fb = get_string('productiondata_assign_private', 'shophandlers_std_addtrainingcredits', $creditrec->coursecredits);
            $productionfeedback->private = $fb;
            $fb = get_string('productiondata_assign_sales', 'shophandlers_std_addtrainingcredits', $creditrec->coursecredits);
            $productionfeedback->salesadmin = $fb;
        } else {
            // Training credits not installed.
            $fb = get_string('productiondata_failure_public', 'shophandlers_std_addtrainingcredits', 'Code : CATEGORY CREATION');
            $productionfeedback->public = $fb;
