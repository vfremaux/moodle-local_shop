    ...
    public function produce_prepay(&$data) {
        ...
        if (isloggedin()) {
            if ($customer->hasaccount != $USER->id) {
                ...
                $message = "[{$data->transactionid}] STD_CREATE_COURSE Prepay :";
            ...
            if (!shop_create_customer_user($data, $customer, $newuser)) {
                $message = "[{$data->transactionid}] STD_CREATE_COURSE Prepay Error :";
            ...
            $fb = get_string('productiondata_sales', 'shophandlers_std_createcourse', $newuser->username);
