...
class shop_paymode_ogone extends shop_paymode{
        ...
        if ($this->check_data_out()) {
            ...
            if ($aFullBill = Bill::get_by_transaction($transid)) {
                ...
                if ($aFullBill->status == SHOP_BILL_PLACED) {
                    ...
                    redirect(new moodle_url('/local/shop/front/view.php', array('view' => $this->theshop->get_next_step('payment'), 'id' => $this->theshop->id, 'transid' => $transid)));
                }
            } else {
            ...
            if ($aFullBill = Bill::get_by_transaction($transid)) {
                ...
                if (in_array($_REQUEST['STATUS'], $acceptancecodes)) {
                    if ($aFullBill->status != SHOP_BILL_SOLDOUT) {
                        ...
                        shop_trace("[{$aFullBill->transactionid}] Ogone/Ingenico IPN Start Production");
                ...
                } elseif (in_array($_REQUEST['STATUS'], $rejectcodes)) {
                ...
                } elseif (in_array($_REQUEST['STATUS'], $cancellationcodes)) {
    ...
    private function check_data_out() {
        ...
        if (empty($checkdata)) return false;
