...
class shop_paymode_sherlocks extends shop_paymode {
    ...
    public function process_ipn() {
        ...
        if ($paydata['code'] != 0 && !empty($paydata['error'])) {
            ...
            $sherlocksapierrorstr = get_string('sherlocksapierror', 'shoppaymodes_sherlocks');
            shop_trace("sherlocks IPN : {$sherlocksapierrorstr} : ".$paydata['error']);
            die;
            ...
            if ($afullbill->status == SHOP_BILL_PENDING || $afullbill->status == SHOP_BILL_PLACED) {
                ...
                if ($paydata['response_code'] == SHLCK_PAYMENT_ACCEPTED) {
                    ...
                    $afullbill->status = SHOP_BILL_REFUSED;
                    $afullbill->save(true);
                    shop_trace("[$transid] sherlocks IPN Payment Rejected : ".$paydata['response_code']);
                    die;
                } else {
                    $afullbill->status = SHOP_BILL_FAILED;
                    $afullbill->save(true); // stateonly

                    shop_trace("[$transid] sherlocks IPN failure : ".$paydata['response_code']);
                    die;
                }
            } else {
                shop_trace("[$transid] sherlocks IPN : Inactive state ".$afullbill->status);
            }
        }
    }
    ...
    public function generate_pathfile() {
        ...
        $pluginpath = $CFG->dirroot.'/local/shop/paymodes/sherlocks/sherlocks_615_PLUGIN_'.$os.$config->sherlocks_processor_type;
    ...
    protected function get_pathfile($os) {
        ...
        if ($os == 'linux') {
            ...
            return str_replace('/', "\\", $CFG->dirroot).'\\blocks\\shop\\paymodes\\sherlocks\\sherlocks_615_PLUGIN_'.$os.$config->sherlocks_processor_type.'\\param\\pathfile';
        }
    }
