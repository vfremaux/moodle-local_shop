...
if ($config->systempay_use_localtime) {
    $parms['vads_trans_date'] =  date('YmdHis'); // 20 chars max / no special chars
} else {
...
$parms['vads_cust_first_name'] = $portlet->customer->firstname;
$parms['vads_cust_last_name'] = $portlet->customer->lastname;
$parms['vads_cust_legal_name'] = $portlet->customer->organisation;
...
<div class="payportlet">
    <form method="POST" name="systempayform" action="<?php echo $url ?>">
        ...
        if ($config->test) {
            echo '<br/><span style="color:red;font-weight:bolder">(Mode Test)</span>';
        }
        echo '</center>';
        ?>
    </form>
...
<?php
    ...
    global $OUTPUT;
    echo $OUTPUT->notification(get_string('errorsystempaynotsetup', 'shoppaymodes_systempay'), 'error');
}
